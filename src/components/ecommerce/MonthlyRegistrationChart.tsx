...
// import Chart from "react-apexcharts";
// import { ApexOptions } from "apexcharts";
// import { useMonthlyRegistrations } from "../../pages/hooks/useMonthlyRegistrations";
...
export default function MonthlyRegistrationChart() {
  const { data, loading, selectedYear, setSelectedYear, availableYears } =
    useMonthlyRegistrations();
  ...
  const series = [
    {
      name: "Service Users",
      data: data?.[selectedYear]?.users ?? Array(12).fill(0),
    },
    {
      name: "Service Providers",
      data: data?.[selectedYear]?.mechanics ?? Array(12).fill(0),
    },
  ];
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-white/5 shadow-sm transition-all">
      ...
      <div className="flex items-center justify-between px-5 pt-5 sm:px-6 sm:pt-6">
        ...
        {!loading && (
          <select
            className="text-sm px-3 py-1 border border-gray-300 rounded-md dark:bg-gray-700 dark:text-white"
            value={selectedYear}
